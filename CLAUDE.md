...
- Documenti della sede (M10.2) in `location_documents` + bucket privato `location-documents`: download solo via `getLocationDocumentUrl` (`src/lib/locations/documents-fns.ts`), che è l'unico punto che usa il client service role; la categoria `template_overlay` non è mai pubblica.
