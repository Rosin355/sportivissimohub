...
- Firma elettronica semplice: `enrollment_signatures` (append-only) + PNG in `documents/{parent}/{iscrizione}/firme/`; regole in `src/lib/enrollments/signatures.ts`, salvataggio in `signatures-fns.ts`, immagini nei PDF via `PdfBuildContext.signatures`. Non obbligatoria; il wizard non la tocca.
