...
- [ ] **M10.1 — Sedi nel database.** Migrare `locations.ts` a tabella `locations` (+ tabelle figlie per settimane, extra, FAQ, documenti richiesti, o jsonb dove sensato) preservando slug e URL esistenti. Campi: tipo (centro estivo/doposcuola/corso/progetto scuola), nome, comune, indirizzo, età, descrizioni, orari, struttura pricing completa (residenti/non residenti, intera/mezza, sconto fratelli, tessere, mora), spots, contatti, stato bozza/pubblicata, logo del comune (upload, mostrato nella pagina pubblica e nell'intestazione dei PDF generati). RLS: lettura pubblica delle pubblicate, scrittura solo admin. Seed della migrazione = dati attuali di locations.ts. Editor admin completo (crea/modifica/bozza/pubblica). Tutto ciò che oggi legge locations.ts passa al DB (lista, dettaglio, wizard, admin, PDF, email).
