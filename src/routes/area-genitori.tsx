...
export const Route = createFileRoute("/area-genitori")({
  beforeLoad: ({ context, location }) => {
    requireRole(context.auth, "genitore", location.href);
  },
      ...
      <main className="flex-1 container mx-auto px-4 py-10">
        ...
        <div className="rounded-2xl bg-gradient-cta-banner text-white p-6 shadow-pop flex items-center gap-4 flex-wrap relative overflow-hidden">
          ...
          <div className="relative flex-1 min-w-[200px]">
            ...
            <p className="text-sm text-white/65 mt-0.5">
              Hai sbloccato il livello "Famiglia Esperta" 🏆
            </p>
          </div>
          <Link
            to="/centri-estivi"
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
          <StatCard
            ...
            value={String(new Set(list.map((e) => e.child.firstName + e.child.lastName)).size || 0)}
          />
          <StatCard
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {list.map((e) => {
              const progress =
                e.status === "confermata"
                  ? 100
                  : e.status === "attesa-pagamento"
                    ? 80
                    : e.status === "revisione"
                      ? 60
                      : e.status === "documenti-mancanti"
                        ? 45
                        : e.status === "nuova"
                          ? 30
                          : 50;
            ...
            {[
              {
                ...
                t: "Foto della settimana!",
                d: "Nuove foto del centro di Galzignano.",
              },
              {
                grad: "bg-gradient-magic",
                t: "Documento da firmare",
                d: "Patto educativo per Sofia.",
              },
              {
                grad: "bg-gradient-grass",
                t: "Iscrizione confermata",
                d: "Marco è in squadra! 🎉",
              },
            ].map((a, i) => (
              <div
                key={i}
