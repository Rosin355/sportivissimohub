...
export const Route = createFileRoute("/centri-estivi/$slug/iscrizione")({
  head: ({ params }) => {
    ...
    const title = loc ? `Iscrivi tuo figlio · ${loc.name} — Sportivissimo` : "Iscrizione — Sportivissimo";
    return { meta: [{ title }, { name: "description", content: "Modulo di iscrizione al centro estivo Sportivissimo." }] };
  },
  loader: ({ params }) => {
    const loc = getLocationBySlug(params.slug);
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-10 max-w-5xl">
        <Link to="/centri-estivi/$slug" params={{ slug: loc.slug }} className="text-sm font-semibold text-muted-foreground hover:text-foreground">
