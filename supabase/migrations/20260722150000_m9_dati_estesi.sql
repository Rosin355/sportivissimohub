...
-- Le righe esistenti restano valide: i nuovi campi sono nullable o con default.

create type public.child_sex as enum ('M', 'F');
create type public.tessera_tipo as enum ('base', 'super_integrativa');

-- children: anagrafica completa + minori senza codice fiscale italiano
...
-- CF italiano di 16 caratteri, oppure estremi completi del documento estero
...
-- enrollments: secondo genitore, residenza, tessera, ordine figlio, consensi ACSI
...
-- Nota: consent_acsi_dati_24 è obbligatorio per le NUOVE iscrizioni (vincolo
-- applicato dalla validazione zod nella server function, non da un check,
-- perché le iscrizioni esistenti hanno false e devono restare valide).
-- Nessuna modifica alle RLS: le policy sono a livello di riga e coprono
